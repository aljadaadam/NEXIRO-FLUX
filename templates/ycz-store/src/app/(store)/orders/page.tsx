...
export default function OrdersPage() {
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '1.5rem 1rem 3rem' }}>
      <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-primary)', marginBottom: 20 }}>{t('📋 سجل الطلبات')}</h2>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
        {filters.map(f => (
          <button key={f} onClick={() => setFilter(f)} style={{
            ...
            background: filter === f ? currentTheme.primary : 'var(--bg-muted)',
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ position: 'relative', flex: '1 1 200px', minWidth: 180 }}>
          <Search size={16} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)', pointerEvents: 'none' }} />
          <input
            type="text"
            value={searchQuery}
            ...
            style={{
              ...
              border: '1px solid var(--border-default)', fontSize: '0.8rem', fontFamily: 'inherit',
        ...
        <button onClick={() => setDateFilter(dateFilter === 'today' ? 'all' : 'today')} style={{
          padding: '0.55rem 0.85rem', borderRadius: 10, border: '1px solid var(--border-default)', cursor: 'pointer',
          background: dateFilter === 'today' ? currentTheme.primary : 'var(--bg-card)',
          color: dateFilter === 'today' ? '#fff' : 'var(--text-secondary)',
          fontSize: '0.78rem', fontWeight: 600, fontFamily: 'inherit',
        ...
        <button onClick={() => setDateFilter(dateFilter === 'range' ? 'all' : 'range')} style={{
          padding: '0.55rem 0.85rem', borderRadius: 10, border: '1px solid var(--border-default)', cursor: 'pointer',
          background: dateFilter === 'range' ? currentTheme.primary : 'var(--bg-card)',
          color: dateFilter === 'range' ? '#fff' : 'var(--text-secondary)',
          fontSize: '0.78rem', fontWeight: 600, fontFamily: 'inherit',
          display: 'flex', alignItems: 'center', gap: 4,
        }}>
      ...
      {dateFilter === 'range' && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          <label style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', fontWeight: 600 }}>{t('من')}</label>
          <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} style={{
            padding: '0.45rem 0.6rem', borderRadius: 8, border: '1px solid var(--border-default)',
            fontSize: '0.8rem', fontFamily: 'inherit', outline: 'none', color: 'var(--text-primary)',
          }} />
          <label style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', fontWeight: 600 }}>{t('إلى')}</label>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {filtered.map(order => {
          ...
          return (
          <div key={order.id} style={{ background: 'var(--bg-card)', borderRadius: 14, padding: '1rem 1.25rem', border: '1px solid var(--border-light)', boxShadow: 'var(--shadow-sm)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <span style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-secondary)' }}>{order.order_number}</span>
            ...
            <p style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: 8, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{order.product_name}</p>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 16px', marginBottom: 8 }}>
                  {fields.map((f, i) => (
                    <span key={i} style={{ fontSize: '0.76rem', color: 'var(--text-secondary)' }}>
                      ...
                      <span style={{ color: 'var(--text-primary)', direction: 'ltr', unicodeBidi: 'embed' }}>{f.value}</span>
                    </span>
                  ))}
                </div>
              ) : null;
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', color: 'var(--text-muted)' }}>
              ...
              <span style={{ fontWeight: 700, color: 'var(--text-primary)' }}>${order.total_price.toFixed(2)}</span>
            </div>
          </div>
          );
        })}
      ...
      {filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-muted)' }}>
          <Package size={48} color="var(--border-default)" style={{ margin: '0 auto 12px', display: 'block' }} />
