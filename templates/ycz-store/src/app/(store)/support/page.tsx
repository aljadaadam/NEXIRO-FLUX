  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '1.5rem 1rem 3rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginBottom: '2rem' }}>
        {[
          ...
          <a key={i} href={m.href} target={m.href.startsWith('https') ? '_blank' : undefined} rel="noopener noreferrer"
            style={{ background: 'var(--bg-card)', borderRadius: 14, padding: '1.25rem', textAlign: 'center', border: '1px solid var(--border-light)', cursor: m.desc !== '—' ? 'pointer' : 'default', textDecoration: 'none', display: 'block', transition: 'box-shadow 0.2s' }}
          ...
          >
            ...
            <h4 style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: 4 }}>{m.title}</h4>
            <p style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', direction: 'ltr', lineHeight: 1.5, overflowWrap: 'anywhere', wordBreak: 'break-word', whiteSpace: 'normal', margin: 0 }}>{m.desc}</p>
          </a>
        ))}
      </div>

      {/* FAQ */}
      <h3 style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: 12 }}>{t('الأسئلة الشائعة')}</h3>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: '2rem' }}>
        {FAQ_DATA.map((faq, i) => (
          <div key={i} style={{ background: 'var(--bg-card)', borderRadius: 12, border: '1px solid var(--border-light)', overflow: 'hidden' }}>
            <button onClick={() => setOpenFaq(openFaq === i ? null : i)} style={{ width: '100%', padding: '1rem 1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: 'none', background: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
              <span style={{ fontSize: '0.88rem', fontWeight: 600, color: 'var(--text-primary)', textAlign: 'right' }}>{t(faq.q)}</span>
              <ChevronDown size={16} color="var(--text-muted)" style={{ transform: openFaq === i ? 'rotate(180deg)' : 'none', transition: 'transform 0.3s', flexShrink: 0 }} />
            </button>
            {openFaq === i && (
              <div style={{ padding: '0 1.25rem 1rem', fontSize: '0.82rem', color: 'var(--text-secondary)', lineHeight: 1.7 }}>{t(faq.a)}</div>
            )}
          </div>
        ))}
      </div>
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: 16, padding: '1.5rem', border: '1px solid var(--border-light)' }}>
        <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: 16 }}>{t('إرسال تذكرة دعم')}</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <input placeholder={t('الموضوع')} style={{ padding: '0.7rem 1rem', borderRadius: 10, border: '1px solid var(--border-default)', fontSize: '0.85rem', fontFamily: 'inherit', outline: 'none' }} />
          <textarea placeholder={t('اكتب رسالتك هنا...')} rows={4} style={{ padding: '0.7rem 1rem', borderRadius: 10, border: '1px solid var(--border-default)', fontSize: '0.85rem', fontFamily: 'inherit', outline: 'none', resize: 'vertical' }} />
