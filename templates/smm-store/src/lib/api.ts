...
async function adminFetch(endpoint: string, options: RequestInit = {}) {
  ...
  if (res.status === 401 || res.status === 403) {
    const isDM = typeof window !== 'undefined' && new URLSearchParams(window.location.search).get('demo') === '1';
...
export function mapBackendProduct(p: Record<string, unknown>): Record<string, unknown> {
  ...
  const normalizeFieldDef = (field: unknown, index: number): ProductCustomField | null => {
    ...
    if (typeof field === 'object') {
      ...
      const label = String(f.label || f.fieldname || f.title || f.name || f.key || f.field || f.description || `حقل ${index + 1}`).trim();
      ...
      const required = requiredValue === undefined ? true : ['1', 'true', 'on', 'yes'].includes(String(requiredValue).toLowerCase());
      return { key, label, placeholder: String(f.placeholder || f.description || `أدخل ${label}`), required };
    }
    return null;
  };

  // SMM-specific platform icons
  ...
  // SMM platform images for cards
  const platformImages: Record<string, string> = {
    'انستغرام': '/platforms/instagram.svg',
    'تيك توك': '/platforms/tiktok.svg',
    'يوتيوب': '/platforms/youtube.svg',
    'تويتر': '/platforms/twitter.svg',
    'فيسبوك': '/platforms/facebook.svg',
    'تلغرام': '/platforms/telegram.svg',
    'سناب شات': '/platforms/snapchat.svg',
    'لينكد إن': '/platforms/linkedin.svg',
  };
  ...
  const groupName = String(p.group_name || 'خدمات أخرى');
  ...
  const rawFieldList = Array.isArray(requiresCustom)
    ...
    : (requiresCustom && typeof requiresCustom === 'object' && Array.isArray((requiresCustom as Record<string, unknown>).fields))
      ...
      : (requiresCustom && typeof requiresCustom === 'object')
        ...
        : [];
  const extraFields = rawFieldList.map(normalizeFieldDef).filter((f): f is ProductCustomField => Boolean(f));
  ...
  if (hasCustomImei) {
    ...
    const imeiLabel = String(cj.customname || 'رابط الحساب').trim() || 'رابط الحساب';
    const alreadyExists = extraFields.some(f => f.key.toLowerCase() === 'imei' || f.key.toLowerCase() === imeiLabel.toLowerCase().replace(/\s+/g, '_'));
    if (!alreadyExists) {
      customFields.push({ key: 'imei', label: imeiLabel, placeholder: cj.custominfo ? String(cj.custominfo) : `أدخل ${imeiLabel}`, required: true });
    }
  }
  customFields.push(...extraFields);

  const icon = groupIcons[groupName.toLowerCase()] || groupIcons[groupName] || '🌐';
  const image = platformImages[groupName] || undefined;
...
export const storeApi = {
  getProducts: async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/products/public`, { cache: 'no-store', headers: { 'Content-Type': 'application/json' } });
  ...
  getBlogPosts: async () => {
    if (isDemoMode()) { const demo = getDemoResponse('/blogs/public', 'GET'); if (demo) return demo; }
    try { const res = await fetch(`${API_BASE}/blogs/public`, { cache: 'no-store', headers: { 'Content-Type': 'application/json' } }); return res.json(); } catch { return { posts: [] }; }
  },
  getBlogPost: async (id: number) => {
    if (isDemoMode()) { const demo = getDemoResponse('/blogs/public', 'GET'); if (demo) { const post = (demo as { posts: Array<{ id: number }> }).posts?.find((p: { id: number }) => p.id === id); return post ? { post } : { post: null }; } }
    try { const res = await fetch(`${API_BASE}/blogs/public/${id}`, { cache: 'no-store', headers: { 'Content-Type': 'application/json' } }); return res.json(); } catch { return { post: null }; }
  },
  ...
  getWalletHistory: () => customerFetch('/customers/wallet/history'),
  changePassword: (oldPassword: string, newPassword: string) => customerFetch('/customers/me/password', { method: 'PATCH', body: JSON.stringify({ old_password: oldPassword, new_password: newPassword }) }),
  getEnabledGateways: () => {
    if (isDemoMode()) { const demo = getDemoResponse('/payment-gateways/enabled', 'GET'); if (demo) return Promise.resolve(demo); }
    return fetch(`${API_BASE}/payment-gateways/enabled`).then(r => r.json());
  },
  getStoreInfo: () => {
    if (isDemoMode()) { const demo = getDemoResponse('/store/info', 'GET'); if (demo) return Promise.resolve(demo); }
    return fetch(`${API_BASE}/store/info`).then(r => r.json());
  },
  getFlashPopup: async () => {
    try { const res = await fetch(`${API_BASE}/customization/store`, { cache: 'no-store', headers: { 'Content-Type': 'application/json' } }); const data = await res.json(); const c = data?.customization; if (c && c.flash_enabled) return c; return null; } catch { return null; }
  },
  initCheckout: async (data: { gateway_id: number; amount: number; currency?: string; description?: string; customer_name?: string; customer_email?: string; type?: 'deposit' | 'purchase'; return_url?: string; cancel_url?: string }) => {
    if (isDemoMode()) { const demo = getDemoResponse('/checkout/init', 'POST', data); if (demo) return demo; }
    const token = getAuthToken();
    const res = await fetch(`${API_BASE}/checkout/init`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(token ? { authorization: `Bearer ${token}` } : {}) }, body: JSON.stringify(data) });
    if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err?.error || `HTTP ${res.status}`); }
    return res.json();
  },
  checkPaymentStatus: async (id: number) => {
    if (isDemoMode()) { const demo = getDemoResponse(`/checkout/status/${id}`, 'GET'); if (demo) return demo; }
    const token = getAuthToken();
    const res = await fetch(`${API_BASE}/checkout/status/${id}`, { headers: { 'Content-Type': 'application/json', ...(token ? { authorization: `Bearer ${token}` } : {}) } });
    if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err?.error || `HTTP ${res.status}`); }
    return res.json();
  },
  checkUsdtPayment: async (id: number, txHash?: string) => {
    if (isDemoMode()) { const demo = getDemoResponse(`/checkout/check-usdt/${id}`, 'POST'); if (demo) return demo; }
    const token = getAuthToken();
    const res = await fetch(`${API_BASE}/checkout/check-usdt/${id}`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(token ? { authorization: `Bearer ${token}` } : {}) }, body: JSON.stringify({ txHash: txHash || undefined }) });
    if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err?.error || `HTTP ${res.status}`); }
    return res.json();
  },
  uploadReceipt: async (id: number, data: { receipt_url: string; notes?: string }) => {
    if (isDemoMode()) return { success: true, message: 'تم رفع الإيصال بنجاح (عرض تجريبي)' };
    const res = await fetch(`${API_BASE}/checkout/receipt/${id}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
    if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err?.error || `HTTP ${res.status}`); }
    return res.json();
  },
  chatStart: async (conversation_id: string, customer_name?: string, customer_email?: string) => {
    ...
    const res = await fetch(`${API_BASE}/chat/public/start`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ conversation_id, customer_name, customer_email }) });
    return res.json();
  },
  chatSend: async (conversation_id: string, message: string, customer_name?: string) => {
    ...
    const res = await fetch(`${API_BASE}/chat/public/send`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ conversation_id, message, customer_name }) });
    return res.json();
  },
  chatMessages: async (conversation_id: string, after?: number) => {
    if (isDemoMode()) return { messages: [] };
    const res = await fetch(`${API_BASE}/chat/public/messages?conversation_id=${conversation_id}${after ? `&after=${after}` : ''}`, { headers: { 'Content-Type': 'application/json' } });
    return res.json();
  },
};
