      ...
      <div className="w-full max-w-xl mx-auto">
        ...
        {step === 5 && result && (
          <div className="animate-fadeIn text-center">
            ...
            <div className="bg-[#13151c] border border-gray-800 rounded-xl p-5 max-w-md mx-auto mb-8 text-start space-y-3">
              ...
              <div className="border-t border-gray-800 pt-3 mt-3">
                <p className="text-gray-400 text-xs font-bold mb-2">{isRTL ? '🔐 بيانات الدخول للوحة التحكم' : '🔐 Admin Login Credentials'}</p>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">{isRTL ? 'البريد الإلكتروني' : 'Email'}</span>
                  <span className="text-white font-mono text-xs" dir="ltr">{ownerEmail}</span>
                </div>
                <div className="flex justify-between text-sm mt-1">
                  <span className="text-gray-500">{isRTL ? 'كلمة المرور' : 'Password'}</span>
                  <span className="text-white font-mono text-xs">{'•'.repeat(ownerPassword.length)}</span>
                </div>
              </div>

              {/* ─── رابط لوحة التحكم الفريد ─── */}
              {result.admin_slug && result.site?.domain && (
                <div className="border-t border-gray-800 pt-3 mt-3">
                  <p className="text-gray-400 text-xs font-bold mb-2">{isRTL ? '🔗 رابط لوحة التحكم (احفظه)' : '🔗 Admin Panel URL (save it)'}</p>
                  <div className="flex items-center gap-2">
                    <code className="flex-1 text-emerald-400 font-mono text-[11px] bg-black/40 rounded-lg px-3 py-2 overflow-x-auto whitespace-nowrap" dir="ltr">
                      {'https://' + result.site.domain + '/admin?key=' + result.admin_slug}
                    </code>
                    <button
                      onClick={() => {
                        navigator.clipboard.writeText('https://' + result.site.domain + '/admin?key=' + result.admin_slug);
                      }}
                      className="px-2.5 py-2 rounded-lg bg-gray-800 hover:bg-gray-700 text-gray-400 text-xs transition-colors flex-shrink-0"
                      title={isRTL ? 'نسخ' : 'Copy'}
                    >📋</button>
                  </div>
                  <p className="text-amber-500/80 text-[10px] mt-2 font-medium">{isRTL ? '⚠️ هذا الرابط هو الطريقة الوحيدة للوصول للوحة التحكم. الدخول من /admin مباشرة لن يعمل.' : '⚠️ This is the only way to access the admin panel. Going to /admin directly will not work.'}</p>
                </div>
              )}
            </div>
