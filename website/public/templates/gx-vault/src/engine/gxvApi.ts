...
export const gxvAdminApi = {
  ...
  updateOrder: (id: string, data: Record<string, unknown>) => gxvAdminFetch(`/orders/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
...
export const gxvStoreApi = {
  ...
  getOrders: () => gxvCustomerFetch('/orders'),
  createOrder: (data: Record<string, unknown>) => gxvCustomerFetch('/orders', { method: 'POST', body: JSON.stringify(data) }),
  getProfile: () => gxvCustomerFetch('/profile'),
  updateProfile: (data: Record<string, unknown>) => gxvCustomerFetch('/profile', { method: 'PUT', body: JSON.stringify(data) }),
  login: (data: { email: string; password: string }) => gxvCustomerFetch('/auth/login', { method: 'POST', body: JSON.stringify(data) }),
  register: (data: { name: string; email: string; password: string }) => gxvCustomerFetch('/auth/register', { method: 'POST', body: JSON.stringify(data) }),
  chargeWallet: (data: Record<string, unknown>) => gxvCustomerFetch('/wallet/charge', { method: 'POST', body: JSON.stringify(data) }),
  getStoreInfo: () => fetch(`${GXV_API_BASE}/store/info`).then(r => r.json()),
};
